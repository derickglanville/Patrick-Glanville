...
function Get-ConfiguredRecipients {
  ...
  try {
    ...
    $Recipients = @($Settings.recipients) | Where-Object { -not [string]::IsNullOrWhiteSpace([string]$_) }
    if ($Recipients.Count) {
      return $Recipients
    }
  } catch {
...
$Recipients = @(Get-ConfiguredRecipients)
